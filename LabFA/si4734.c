...
uint8_t si4734_rd_buf[15];         //buffer for holding data received from the si4734
...
extern char uart1_tx_buf[40];      //holds string to send to UART
...
void fm_tune_freq(){
  ...
  si4734_wr_buf[4] = 0x00;  //antenna tuning capacitor
...
void am_tune_freq(){
  ...
  si4734_wr_buf[4] = 0x00;  //antenna tuning capacitor high byte
  si4734_wr_buf[5] = 0x00;  //antenna tuning capacitor low byte
...
//antcap low byte is 0x01 as per data sheet
...
void sw_tune_freq(){
  ...
  si4734_wr_buf[4] = 0x00;  //antenna tuning capacitor high byte
  si4734_wr_buf[5] = 0x01;  //antenna tuning capacitor low byte 
  //send am tune command
  twi_start_wr(SI4734_ADDRESS, si4734_wr_buf, 6);
...
// restore the previous FM frequency  
 ...
 //current_volume  = eeprom_read_byte(&eeprom_volume); //TODO: only this one does not work 
  ...
  si4734_wr_buf[0] = FM_PWR_UP; //power up command byte
