...
void spi_init(void){
  ...
  SPCR  |=   (1<<SPIE)|(1<<MSTR);         //enable SPI, master mode 
...
ISR(TIMER0_OVF_vect){
  ...
  if ((count_7ms % 500)==0){            //?? interrupts equals one half second 
    SPDR = count_7ms;                    //send to display 
  ...
  if (display_count == 7680){display_count = 0;} //back to 1st positon
}
